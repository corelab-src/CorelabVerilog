...
std::set<int> &PointerAnalysis::pointsTo(int A) {
	if (debug)
		std::cerr << "Recovering Points-to-set of " << A << std::endl;
...
//		while (Z != from[Node].end() ) 
		auto count_max = from[Node].size();
		unsigned count = 0;
		if (debug) std::cerr << "COUNT MAX : " << count_max << "\n";
//		for ( auto Z : from[Node] )
		while(1)
		{
			if ( count == count_max )
				break;

			if (debug) std::cerr << "comparing step start\n";
...
//			int ZVal = Z;
	...
	if (debug) std::cerr << "Consolidating\n";
	...
	if (debug) std::cerr << "Consolidate Done\n";
}

// ============================================= //

/// Prints the graph to std output
void PointerAnalysis::print(raw_ostream &O) {
	O << "# of Vertices: ";
	O << vertices.size() << "\n";
	IntSet::iterator it;
	IntMap::iterator v;
	...
	for (v = vertices.begin(); v != vertices.end(); v++) {
		O << v->first << " -> {";
		IntSet::iterator n;
		for (n = pointsToSet[v->first].begin();
				n != pointsToSet[v->first].end(); n++) {
			O << *n << ", ";
		}
...
DenseMap<int, std::set<int>> &PointerAnalysis::allPointsTo() {
	return pointsToSet;
}
